...
4. **The snapshot lives under `generated/v4/`.** The older v3 snapshots (full Action mirror) live under `generated/v3/`. Don't mix imports across versions.
...
```
lib/ontology-gen/
├── (v3 codegen — root files, untouched)
└── v4/
    ├── assemble-v4-4.ts            # immutable; passes runtimeInput verbatim when it's a string
    ├── assemble.ts                 # RUNTIME_INPUT_PLACEHOLDER constant
    ├── runtime.ts                  # legacy strategy router (v4-1/2/3/4) — still works
    ...
    └── index.ts                    # exports generatePrompt / fillRuntimeInput + types

generated/
├── v3/                             # 22 legacy snapshots (full Action mirror)
│   ├── action-object.types.ts
│   └── <action>.action-object.ts
└── v4/                             # ★ new
    ├── action-object-v4.types.ts   # ★ consumer ABI (hand-written subset)
...
- `lib/ontology-gen/v4/runtime.ts` — legacy `resolveActionObjectV4` strategy router. Still works for `app/dev/action-preview/`.
- `lib/ontology-gen/fetch.ts` — fetch + invariant assertions; same path as v3.
- The v4-1/2/3 strategies and their templates — kept for legacy paths and the dev preview page.
...
- **`generated/v3/` is a new convention.** Any code still importing from `@/generated/<name>.action-object` (root-level) will 404 — update to `@/generated/v3/<name>.action-object`. Fixtures and v3 generation scripts have been updated in this iteration; if you find a forgotten consumer, point it at v3/.
...
- Spec: `docs/2026-05-08-action-object-codegen.spec.md` (v3 codegen design)
- Ontology API: `ONTOLOGY-API-USER-GUIDE-BASED-ON-NEO4J.md`
- v3 user guide: `ACTION-OBJECT-CODEGEN-USER-GUIDE.md`
- Plan record: `~/.claude/plans/runtime-input-enchanted-teacup.md`
