/* global React, useT, AppBar, LeftNav, Ic, DirectionTag, CommandPalette, StatusDot */

// ===== Direction D: Event Manager =====
// Inngest-style event bus UI. Left: registry, Center: detail w/ tabs, Right: live stream.

// Event catalog — mirrors events.json. Each entry carries just enough to drive the UI.
const EVENT_CATALOG = [
  // Requirement
  { name: "SCHEDULED_SYNC",          stage: "system",     kind: "trigger", rate: 6,   err: 0,    desc: "定时任务触发信号，驱动系统自动拉取客户 RMS 变更。",
    ...
    data: [
      ["client_rms_system_id", "String"],   ["sync_scope", "Array"],
      ["last_successful_sync_at", "String"],["sync_frequency_minutes", "Integer"],
  ...
  { name: "SYNC_FAILED_ALERT",       stage: "system",      kind: "error",   rate: 2,   err: 2,    desc: "接口异常或数据校验失败导致需求同步中断，触发预警。",
    publishers: ["ReqSync"], subscribers: ["Alerting","HSM·值班台","AuditLogger"], wf: ["incident/v1"],
    emits: [],
    data: [["sync_result","String"],["synced_job_requisition_ids","List<String>"]],
    mutations: ["Job_Requisition_Specification","Job_Requisition","Client"] },
  ...
  { name: "JD_GENERATED",            stage: "jd", kind: "domain", rate: 74,  err: 0,  desc: "基于已确认的需求，系统自动生成的标准化 JD 已就绪，等待审核。",
    ...
    data: [["job_posting_id","String"],["jd_content","String"]],
    mutations: ["Job_Posting"] },
  ...
  { name: "RESUME_PROCESSED",        stage: "resume", kind: "domain", rate: 2802, err: 0, desc: "简历解析已完成（或人工修复），结构化数据准备进入匹配环节。",
    ...
    data: [["fix_result","String"]],
    mutations: ["Resume","Candidate"] },
  ...
  { name: "RESUME_LOCKED_CONFLICT",  stage: "resume", kind: "error",  rate: 18,   err: 18, desc: "处理时检测到唯一性归属冲突，当前流程中止。",
    ...
    data: [["candidate_id","String"],["resume_id","String"],["process_status","String"]],
    mutations: ["Candidate","Resume"] },
  { name: "RESUME_OPTIMIZED",        stage: "resume", kind: "domain", rate: 88,   err: 0,  desc: "简历内容已完成亮点提炼与格式美化，为推荐包前置条件。",
    publishers: ["ResumeOptimizer"], subscribers: ["PackageBuilder"], wf: ["package/v2"],
    emits: ["PACKAGE_GENERATED"],
    data: [["candidate_id","String"],["original_resume_id","String"],["optimized_content_summary","Object"],["optimization_strategy","String"],["generation_engine","String"]], mutations: [] },
  ...
  { name: "PACKAGE_MISSING_INFO",        stage: "package", kind: "gate",   rate: 12, err: 0, desc: "生成推荐包时发现必要材料（作品集、评估项）不全。",
    publishers: ["PackageBuilder"], subscribers: ["HSM·补全台"], wf: ["package-enrich/v1"],
    emits: ["PACKAGE_GENERATED"],
    data: [["recommendation_material_id","String"],["package_status","String"]],
    mutations: ["Recommendation_Material"] },
...
const STAGE_LABELS = {
  requirement: "eg_requirement", jd: "eg_jd", resume: "eg_resume",
  match: "eg_match", interview: "eg_interview", eval: "eg_eval",
  package: "eg_package", submit: "eg_submit", system: "eg_system",
};

function kindDot(kind) {
  switch (kind) {
    case "error":   return "var(--c-err)";
    case "gate":    return "oklch(0.5 0.14 75)"; // warn ink
    case "trigger": return "var(--c-info)";
    default:        return "var(--c-ok)";
  }
}

function EventManagerArtboard() {
  const { t } = useT();
  ...
  // Group events by stage for the registry
  ...
  return (
    <div className="ao-frame" data-screen-label="D Event Manager">
      <AppBar
        crumbs={[t("nav_group_build"), t("em_registry"), selected.name]}
        onOpenCmdK={() => setCmdkOpen(true)}
      />
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        <LeftNav active="triggers" />
      ...
      <DirectionTag label={t("dirD")} />
      <CommandPalette open={cmdkOpen} onClose={() => setCmdkOpen(false)} />
    </div>
  );
}

// ===== Sub-header with system stats =====
function EMSubHeader() {
  ...
  return (
    <div style={{ padding: "14px 22px", borderBottom: "1px solid var(--c-line)", background: "var(--c-surface)", display: "flex", alignItems: "center", gap: 18 }}>
      <div>
        <div style={{ fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em" }}>{t("em_title")}</div>
        <div className="muted" style={{ fontSize: 12, marginTop: 1 }}>{t("em_sub")}</div>
      </div>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(6, minmax(0,1fr))", gap: 14, paddingLeft: 18, borderLeft: "1px solid var(--c-line)" }}>
        {stats.map((s, i) => (
          <div key={i}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, letterSpacing: "-0.01em", fontFeatureSettings: '"tnum"' }}>{s.value}</div>
            <div className="mono" style={{ fontSize: 10.5, color: s.tone === "up" ? "var(--c-ok)" : s.tone === "down" ? "var(--c-err)" : "var(--c-ink-4)" }}>
              {s.delta}
            </div>
          </div>
        ))}
      </div>
      <button className="btn sm"><Ic.plus /> 新建事件</button>
      <button className="btn sm primary"><Ic.bolt /> 发布事件</button>
    </div>
  );
}

// ===== Left: Event Registry =====
function EventRegistry({ grouped, selectedName, onSelect, query, setQuery }) {
  const { t } = useT();
    ...
    <aside style={{ borderRight: "1px solid var(--c-line)", background: "var(--c-surface)", display: "flex", flexDirection: "column", minHeight: 0 }}>
      <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--c-line)" }}>
        <div className="row" style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 13, fontWeight: 600, flex: 1 }}>{t("em_registry")}</div>
          <span className="badge">{totalCount} · {EVENT_CATALOG.length}</span>
        </div>
        <div style={{ position: "relative" }}>
          <span style={{ position: "absolute", left: 8, top: 6, color: "var(--c-ink-4)" }}><Ic.search /></span>
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="EVENT_NAME_…"
            style={{
              width: "100%", height: 28, border: "1px solid var(--c-line)", background: "var(--c-panel)",
              borderRadius: 5, padding: "0 8px 0 28px", fontFamily: "var(--f-mono)",
              fontSize: 11.5, color: "var(--c-ink-1)", outline: "none",
            }}
          />
        </div>
      </div>
          ...
          return (
            <div key={stage}>
              <div style={{
                ...
                color: "var(--c-ink-4)", fontWeight: 600,
                background: "var(--c-panel)", borderTop: "1px solid var(--c-line)", borderBottom: "1px solid var(--c-line)",
                display: "flex", alignItems: "center",
              }}>
                <span style={{ flex: 1 }}>{t(STAGE_LABELS[stage])}</span>
                <span className="mono" style={{ color: "var(--c-ink-4)", fontWeight: 500 }}>{items.length}</span>
              </div>
              {items.map((e) => (
...
function RegistryRow({ event, active, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      <span style={{ width: 6, height: 6, borderRadius: "50%", background: kindDot(event.kind), flexShrink: 0, boxShadow: `0 0 0 3px color-mix(in oklab, ${kindDot(event.kind)} 18%, transparent)` }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="mono" style={{ fontSize: 11, fontWeight: 600, color: active ? "var(--c-accent)" : "var(--c-ink-1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{event.name}</div>
        <div className="mono" style={{ fontSize: 10, color: "var(--c-ink-4)", marginTop: 1 }}>
          {event.rate.toLocaleString()}/h · {event.subscribers.length} sub{event.err > 0 && <span style={{ color: "var(--c-err)" }}> · {event.err} err</span>}
        </div>
      </div>
    </div>
  );
...
window.EventManagerArtboard = EventManagerArtboard;
window.EVENT_CATALOG = EVENT_CATALOG;
window.STAGE_LABELS = STAGE_LABELS;
window.kindDot = kindDot;
